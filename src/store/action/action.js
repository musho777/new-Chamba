...
export const GetLentsAction = (
  token,
  page = 'https://chambaonline.pro/api/lents?page=1',
) => {
  var myHeaders = new Headers();
  myHeaders.append('Content-Type', 'application/json');
  myHeaders.append('Authorization', `Bearer ${token}`);
...
export const GetPostViewAction = (data, token, page) => {
  ...
  return dispatch => {
    ...
    fetch(
      `${Api}/get_post_vied_user_and_statistics?page=${page}`,
      requestOptions,
    )
      .then(response => response.json())
      .then(r => {
...
export const GetStatisitc2 = (id, token, data) => {
  ...
  return dispatch => {
    ...
    fetch(
      `${Api}/get_statistics_three_with_date_new?post_id=${id}`,
      requestOptions,
    )
      .then(response => response.json())
      .then(r => {
