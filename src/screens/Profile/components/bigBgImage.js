...
export const BigBgImage = forwardRef(
  (
    {changeAvatar, setChangeAvatar, user, bg, bgPhoto, imgUrl, setOpenBg},
    ref,
  ) => {
          ...
          <Text
            style={[
              Styles.darkMedium16,
              {
                textAlign: 'center',
                paddingTop: 15,
                color: JSON.parse(user?.name)?.color?.title
                  ? JSON.parse(user?.name)?.color?.title
                  : 'black',
                fontFamily: JSON.parse(user?.name)?.font,
              },
            ]}>
            {JSON.parse(user?.name)?.name}
          </Text>
        );
      } catch (error) {
        return (
        ...
        <View style={{width: '100%'}}>
          <TouchableOpacity
            ...
            <FastImage
              ...
              source={{
                uri: bg ? bg : `https://chambaonline.pro/uploads/${bgPhoto}`,
              }}
            />
          </TouchableOpacity>
          <TouchableOpacity
            ...
            <Text
              style={[
                Styles.darkMedium14,
                {
                  width: '100%',
                  textAlign: 'center',
                  color: JSON.parse(user?.description)?.color.title,
                  fontFamily: JSON.parse(user?.description)?.font,
