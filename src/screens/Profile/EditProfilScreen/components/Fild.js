  ...
  if (discription) {
    return <View >
      ...
      <View style={{ marginTop: 10 }}>
        <Text style={[Styles.balihaiMedium10, { paddingHorizontal: 17,color:"#000" }]}>Выбери уникальный шрифт и цвет для описания о себе.</Text>
        <ScrollView showsHorizontalScrollIndicator={false} horizontal contentContainerStyle={{ gap: 10, paddingHorizontal: 17, alignItems: 'center', marginVertical: 10 }}>
          {fontFamily.map((elm, i) => {
            return <Text onPress={() => {
              ...
              setActicveFont(elm)
            }} key={i} style={{ fontSize: 10, fontFamily: elm,color:"#000" }}>{elm}</Text>
          })}
        </ScrollView>
  ...
  return <View >
    <View style={[styles.textWrapper, { borderBottomWidth: bB }]}>
      ...
      <TextInput
        ...
        value={value}
        onChangeText={e => ChnageData(e, activeFont, activeColor)}
        style={[Styles.balihaiMedium14, { height: 'auto', width: '90%', fontFamily: activeFont, paddingHorizontal: 0,color:"#000" }]}
      />
    </View>
  </View>
}
