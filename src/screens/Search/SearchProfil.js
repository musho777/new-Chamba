...
export const SearchProfil = ({navigation, route}) => {
        ...
        <Text
          style={[
            Styles.darkMedium16,
            {
              textAlign: 'center',
              paddingTop: 15,
              color: JSON.parse(data.data?.name)?.color?.title,
              fontFamily: JSON.parse(data?.data.name)?.font,
            },
          ]}>
          {JSON.parse(data?.data.name)?.name}
        </Text>
      );
    } catch (error) {
      return (
        <Text
          style={[Styles.darkMedium16, {textAlign: 'center', paddingTop: 15}]}>
          {data?.data?.name}
        </Text>
      );
    }
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const result = await response.json();
        let index = result?.data.follow_status_sender?.findIndex(
          elm => elm.sender_id == user.data.id,
        );
        setIsFollow(index >= 0);
  ...
  const ListEmptyComponent = () => {
    return (
      <EmptyFlatlist
        loading={postLoading}
        text={t(mainData.lang).Thefeedisempty}
      />
    );
  };
  ...
  return (
    <SafeAreaView style={{flex: 1}}>
      <FlatList
        ...
        ListHeaderComponent={
          <>
              ...
              <View style={{width: width}}>
                <TouchableOpacity
                  ...
                  <FastImage
                    ...
                    source={{
                      uri: `https://chambaonline.pro/uploads/${data?.data.backround_photo}`,
                    }}
                  />
                </TouchableOpacity>
                <View style={styles.avatarWrapper1} activeOpacity={1}>
                  <TouchableOpacity
                    ...
                    style={[styles.shadow, styles.avatar]}>
                    <Image
                      style={styles.img}
                      source={{
                        uri: `https://chambaonline.pro/uploads/${data?.data.avatar}`,
                      }}
                    />
                  </TouchableOpacity>
                </View>
                  ...
                  <Text
                    style={[
                      Styles.darkMedium14,
                      {
                        width: '100%',
                        textAlign: 'center',
                        color: JSON.parse(data?.data.description).color.title,
                        fontFamily: JSON.parse(data?.data.description).font,
                      },
                    ]}>
            ...
            <AlbomAndInfo
              setSelectedScreen={e => setSelectedScreen(e)}
              seletedScreen={seletedScreen}
            />
          </>
        }
        ListFooterComponent={
      ...
      {openBg && (
        <SliderModal
          modalVisible={openBg}
          photo={[{photo: data?.data.backround_photo}]}
          close={() => setOpenBg(false)}
        />
      )}
    </SafeAreaView>
  );
