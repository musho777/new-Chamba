...
export const EditPostScreen = ({route}) => {
  ...
  // Handle new layout format case
  const initializeDescription = desc => {
    if (typeof desc === 'string' && desc.startsWith('{"descriptions":')) {
      try {
        ...
        console.log('Error parsing layout description:', error);
      }
    }
    return desc;
  };
  ...
  const [description, setDescription] = useState(
    initializeDescription(route.params.description),
  );
  ...
  const EditPost = () => {
    ...
    if (
      typeof route.params.description === 'string' &&
      route.params.description.startsWith('{"descriptions":')
    ) {
      try {
        const originalParsed = JSON.parse(route.params.description);
        if (originalParsed.layout) {
    ...
    if (route.params.big) {
      navigation.navigate('SinglPageScreen', {
        id: route.params.id,
        description: finalDescription,
      });
    } else {
  ...
  const changeDescription = e => {
    ...
    if (Array.isArray(description)) {
      ...
      const updatedDescriptions = [...description.descriptions];
      updatedDescriptions[index] = e;
      setDescription({
        ...description,
  ...
  useEffect(() => {
    if (Array.isArray(description)) {
      // Case 1: description is an array
      ...
      // Case 2: description is an object with `descriptions` array
      setActiveDescription(description.descriptions[index]);
    }
  }, [index, description]);
  const delateFoto = (i, id, post_id) => {
    ...
    if (description) {
      if (Array.isArray(description)) {
        ...
        setActiveDescription(updatedDescriptions[0] || '');
      } else if (
        typeof description === 'object' &&
        Array.isArray(description.descriptions)
      ) {
    ...
    if (updatedPhotos.length === 0) {
      dispatch(DeletLocalPhoto({post_id}));
    }
      ...
      <View style={{flexDirection: 'row'}}>
        {activeDescription && (
          <View
            style={{
              marginVertical: 10,
              backgroundColor: GetCveta(fonColor),
              borderRadius: 5,
              marginHorizontal: 5,
              paddingBottom: 3,
            }}>
              ...
              <Text
                style={[
                  Styles.darkMedium13,
                  {
                    ...
                    fontFamily: GetFont(fonts),
                    backgroundColor: GetBegraund(begraund),
                    marginTop: 3,
                  },
                ]}>
                {activeDescription}
          ...
          <View>
            <ScrollView
              ...
              {fontFamily.map((elm, i) => {
                return (
                  <Text
                    onPress={() => changeFont(elm, index)}
                    key={i}
                    style={[
                      Styles.darkMedium12,
                      {color: 'black', fontFamily: elm},
                    ]}>
                    {elm}
                  </Text>
                );
          ...
          <View>
            <ScrollView
              ...
              {color.map((elm, i) => {
                return (
                  <TouchableOpacity
                    onPress={() => changeColor(elm.title, index)}
                    key={i}
                    style={{
                      width: 20,
                      height: 20,
          ...
          <View>
            <ScrollView
              ...
              {color2.map((elm, i) => {
                return (
                  <TouchableOpacity
                    onPress={() => chnageBegraund(elm.title, index)}
                    key={i}
                    style={{
                      width: 20,
                      height: 20,
          ...
          <View>
            <ScrollView
              ...
              <TouchableOpacity
                ...
                onPress={() => changeFonColor('rgba(0,0,0,0.5)', index)}>
                <Text style={{fontSize: 15}}>x</Text>
              </TouchableOpacity>
              {color3.map((elm, i) => {
                return (
                  <TouchableOpacity
                    onPress={() => changeFonColor(elm.title, index)}
                    key={i}
                    style={{
                      width: 20,
                      height: 20,
