...
export const Slider = React.memo(({ adminStatus, scroll, id, photo, viewableItems, setOpenModal, user, onLongClikc, long, onPressOut, setActiveImage, data, setHoriznotal = () => { }, description, layout = 'vertical', setIsExpanded, isExpanded, setHeight, big, color, font, setPhotoLayout = () => { } }) => {
  ...
  const detectPhotoOrientation = (photos) => {
    if (!photos || photos.length === 0) return 'vertical';
...
<<<<<<< HEAD
      {photo?.length > 0 ? (
        <View>
          {/* Debug info */}
          {__DEV__ && (
            <View style={{position: 'absolute', top: 5, left: 5, zIndex: 1000, backgroundColor: 'rgba(0,0,0,0.7)', padding: 5, borderRadius: 3}}>
              <Text style={{color: 'white', fontSize: 9}}>Макет: {layout || detectedLayout}</Text>
              <Text style={{color: 'white', fontSize: 9}}>Фото: {photo?.length}</Text>
            </View>
          )}
          {renderLayout()}
        </View>
      ...
      {(layout === 'vertical' || detectedLayout === 'vertical') && photo?.length > 1 && photo?.length <= 5 &&
=======
      <FlatList
        horizontal
        ref={flatListRef}
