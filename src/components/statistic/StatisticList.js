...
export const StatisticList = ({id, token, vidio}) => {
  ...
  useEffect(() => {
    if (id) {
      ...
      dispatch(
        GetStatisitc2(id, token, {timezone_offset: getTimezoneOffset()}),
      );
    }
  }, [id]);
  ...
  return (
    <BottomSheetScrollView showsVerticalScrollIndicator={false}>
      <Text style={[{textAlign: 'center'}, Styles.darkMedium16]}>
        СТАТИСТИКА
      </Text>
      <View
        style={{
        ...
        <Accordion
          headerTitleStyle={[Styles.darkMedium12, {textAlign: 'center'}]}
          headerTitle="Статистика по публикации ↓">
          <View style={{gap: 10, marginTop: 20}}>
            <Text style={Styles.darkSemiBold14}>
              Лайков - {getStatistic1.data.get_like_count}
            </Text>
            <Text style={Styles.darkSemiBold14}>
              Комментариев - {getStatistic1.data.get_comment_count}
            </Text>
            <Text style={[Styles.darkSemiBold14]}>
              Поделились аккаунтом - 0
            </Text>
            <Text style={Styles.darkSemiBold14}>
              Просмотров - {getStatistic1.data.get_view_count}
            </Text>
            {!vidio && (
              <View style={{gap: 10}}>
                <Text style={Styles.darkSemiBold14}>
                  Минимальное время просмотра - {getMinView}{' '}
                </Text>
                <Text style={Styles.darkSemiBold14}>
                  Среднее время просмотра - {getView}{' '}
                </Text>
                <Text style={Styles.darkSemiBold14}>
                  Максимальное время просмотра - {getMaxView}{' '}
                </Text>
              </View>
            )}
            ...
            {vidio && (
              <View style={{gap: 10}}>
                <Text style={Styles.darkSemiBold14}>
                  Минимальное время просмотра видео -{' '}
                  {getVidioStatistic.data.min}{' '}
                </Text>
                <Text style={Styles.darkSemiBold14}>
                  Среднее время просмотра видео - {getVidioStatistic.data.avg}{' '}
                </Text>
                <Text style={Styles.darkSemiBold14}>
                  Максимальное время просмотра видео -{' '}
                  {getVidioStatistic.data.max}{' '}
                </Text>
              </View>
            )}
            <Text style={Styles.darkSemiBold14} t>
              Сохранение публикации в закладки -{' '}
              {getStatistic1.data.get_book_count}{' '}
            </Text>
            <Text style={Styles.darkSemiBold14}>
              Переход с ленты на Ваш аккаунт -{' '}
              {getStatistic1.data.get_post_page_count}{' '}
            </Text>
            <TouchableOpacity
              ...
              <Text style={Styles.darkSemiBold14}>
                Просмотрели предыдущие публикации
              </Text>
              <View>
                <DonwSvg />
              </View>
            ...
            <View style={{gap: 10}}>
              <Text style={Styles.darkSemiBold14}>
                Среднее время проведенное на аккаунте - {getViewInAccaunt}
              </Text>
            </View>
            <View>
              <View style={styles.line} />
              {getStatistic1.data?.city_data?.length > 0 && (
                <Text style={[Styles.darkSemiBold14, {marginBottom: 5}]}>
                  Просмотры с городов
                </Text>
              )}
        ...
        <Accordion
          headerTitleStyle={[Styles.darkMedium12, {textAlign: 'center'}]}
          headerTitle="Активность просмотров ↓">
          <ScrollView
          ...
          <Text
            style={[
              {marginTop: 20, marginBottom: 10, paddingHorizontal: 15},
